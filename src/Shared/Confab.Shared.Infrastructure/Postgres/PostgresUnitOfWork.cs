    ...
    public abstract class PostgresUnitOfWork<T> : IUnitOfWork where T : DbContext
    {
        ...
        private async Task RollbackWithLogsAsync(IDbContextTransaction transaction, Exception exContext)
        {
            try
            {
                ...
                _logger.LogWarning(exContext, $"Rollback DB transaction {transaction.TransactionId} applied.");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Rollback DB transaction {transaction.TransactionId} failed.");
                throw;
            }
        }
    }
