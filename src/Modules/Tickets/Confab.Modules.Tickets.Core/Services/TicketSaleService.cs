...
internal class TicketSaleService : ITicketSaleService
{
    ...
    public async Task AddAsync(TicketSaleDto dto)
    {
        ...
        {
            var ticketsCount = await _ticketRepository.GetCountForConferenceAsync(conference.Id);
            var totalTicketsCount = ticketsCount + dto.Amount;
            if (totalTicketsCount > conference.ParticipantsLimit)
        ...
        _logger.LogInformation("Added a ticket sale conference with ID: " +
                               $"'{conference.Id}' ({dto.From} - {dto.To}).");

        if (ticketSale.Amount.HasValue)
        {
            _logger.LogInformation($"Generating {ticketSale.Amount} tickets for conference " +
    ...
    private TicketSaleInfoDto Map(TicketSale ticketSale, Conference conference)
    {
        ...
        return new(ticketSale.Name, Conference: new(conference.Id, conference.Name),
